...
Below is the current flow of `structure_to_iupac/namer.py` after the refactor. At a high level, `namer.py` is now an orchestrator: it parses a molecule, splits it into graph components, names each component, recursively names substituent subgraphs when needed, then assembles the final name.
