...
@pytest.mark.parametrize("length", (1, 2, 4))
@pytest.mark.parametrize("reverse", (False, True))
def test_saturated_n_alkyl_ligand_does_not_consume_carbon_hydrogenation(length, reverse):
    graph = Chem.RWMol()
    for symbol in ("N", "C", "C", "C", "C", "N", "C", "N"):
        graph.AddAtom(Chem.Atom(symbol))
    for u, v, order in (
        (0, 1, 1),
        (1, 2, 1),
    ...
    if reverse:
        mol = Chem.RenumberAtoms(mol, list(reversed(range(mol.GetNumAtoms()))))
    named = name_mol(mol, include_trace=True)
    assert named.error is None
    assert named.parent_nomenclature == "systematic_fusion"
    assert "4,6-dihydro-1H-pyrrolo[3,4-d]imidazole" in named.name
    if opsin_available():
        check = verify_with_opsin(named.name, Chem.MolToSmiles(mol), standardize_smiles=False)
