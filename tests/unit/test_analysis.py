...
from openclatura.heteroatom_substituent_specs import central_oxo_substituent_prefix, unsubstituted_prefix
from openclatura.heterocumulene_roles import nitrogen_heterocumulene_role
from openclatura.hypervalent_roles import HypervalentLigandRole, hypervalent_center_role
from openclatura.ionic_naming import (
    INVALID_LOCANT_IDE_PATTERNS,
...
from openclatura.locants import as_display_locant, locant_text, parse_locant
...
from openclatura.naming_data import namer_rules
...
from openclatura.rule_layout import rule_groups, section_group_map
...
def load_test_json(relative_path: str) -> dict:
...
def test_hydrogen_cyanide_absorbed_nitrile_name_passes_final_audit():
    ...
    assert hydrogen["source"] == "typed_rewrite"
    assert hydrogen["confidence"] == "derived"
    assert hydrogen["ownership"] == "exact"
...
def test_carbonyl_prefix_morphology_is_bound_from_renderer_data():
    analysis = analyze_smiles("NC(=O)c1ccc(C(=O)O)cc1")
    assembly = next(step for step in analysis.decisions if step.decision == "assembled component name")
    carbamoyl = next(token for token in assembly.data["name_token_spans"] if token["text"] == "carbamoyl")

    # ``H2N-CO-`` is carbamoyl, and the prefix covers the whole group.
    assert analysis.name == "4-carbamoylbenzoic acid"
    assert carbamoyl["source"] in {"functional_prefix_renderer", "substituent_renderer"}
    assert carbamoyl["confidence"] == "derived"
    assert carbamoyl["ownership"] == "exact"
    assert carbamoyl["atoms"] == [0, 1, 2]
...
def test_heteroatom_shortcut_leaf_tokens_and_tree_are_graph_local():
    ...
    # P-66.1.1.4.1: the sulfonamide is the principal group, not a sulfonyl prefix on the amine.
    assert analysis.name == "4-amino-N-(4,6-dimethylpyrimidin-2-yl)benzene-1-sulfonamide"
    assert tokens[("N", name.index("N-("))]["atoms"] == [3]
    assert tokens[("N", name.index("N-("))]["source"] == "n_substituent_locant"
    assert tokens[("amino", name.index("amino"))]["atoms"] == [16]
    assert tokens[("dimethylpyrimidin", name.index("dimethylpyrimidin"))]["atoms"] == [4, 5, 6, 7, 8, 9, 10, 11]
    assert tokens[("sulfonamide", name.index("sulfonamide"))]["atoms"] == [0, 1, 2, 3, 12]
    assert tokens[("sulfonamide", name.index("sulfonamide"))]["bonds"] == [1, 2, 3, 12]

    component = analysis.substituent_tree[0]
    ligand = next(item for item in component["substituents"] if item["name"] == "(4,6-dimethylpyrimidin-2-yl)")
    assert ligand["locants"] == ["N"]
    assert ligand["substituents"][0]["name"] == "methyl"
    assert sorted(ligand["substituents"][0]["locants"]) == ["4", "6"]
...
def test_postprocessing_rewrites_keep_token_metadata_auditable():
    result = NameAssemblyResult.from_raw_name(
        "methyloxy",
        [NameAtomBinding(stage="prefix", role="substituent", term="methyloxy", atom_ids={0, 1}, bond_ids={0})],
        postprocess=post_process_name,
    )

    assert result.text == "methoxy"
    ...
    assert {token.text for token in result.token_spans} == {"methoxy"}
...
def test_postprocessing_rewrites_bindings_to_final_emitted_direction():
    # The binding has to follow the spelling that survived post-processing.
    result = NameAssemblyResult.from_raw_name(
        "2-methyloxypropanamide",
        [NameAtomBinding(stage="prefix", role="substituent", term="methyloxy", atom_ids={1, 2})],
        postprocess=post_process_name,
    )

    assert result.text == "2-methoxypropanamide"
    assert result.bindings[0].term == "methoxy"
    assert all(token.binding_indices for token in result.token_spans)


def test_postprocessing_updates_binding_terms():
    bindings = [NameAtomBinding(stage="prefix", role="substituent", term="methyloxy", atom_ids={0, 1})]

    processed = postprocess_name_atom_bindings(bindings, post_process_name)

    assert processed[0].term == "methoxy"
    assert processed[0].atom_ids == {0, 1}


def test_name_assembly_result_preserves_binding_metadata_through_postprocessing():
    bindings = [NameAtomBinding(stage="prefix", role="substituent", term="methyloxy", atom_ids={0, 1}, bond_ids={0})]

    result = NameAssemblyResult.from_raw_name("methyloxy", bindings, postprocess=post_process_name)

    assert result.text == "methoxy"
    assert result.bindings[0].term == "methoxy"
    assert result.bindings[0].atom_ids == {0, 1}
    assert result.bindings[0].bond_ids == {0}
    assert result.rewrite_history[0].changed_binding_count == 1
    assert result.rewrite_history[0].changed_token_count >= 1
    assert all(token.binding_indices for token in result.token_spans)
    assert next(token for token in result.token_spans if token.text == "methoxy").atom_ids == frozenset({0, 1})
...
def test_mixed_numeric_and_element_locants_are_bound_individually():
    ...
    assert analysis.name == "N,2-dimethylpropanamide"
    for key in [("N", 0), ("2", 2)]:
        token = tokens[key]
...
def test_purine_diones_cite_added_hydrogen_as_a_hydro_prefix():
    ...
    assert name_smiles("O=c1[nH]cnc2[nH]cnc12") == "1,9-dihydro-6H-purin-6-one"
...
def test_ketone_claims_the_indicated_hydrogen_at_its_own_position():
    # Pyrrolizine's default indicated hydrogen is 1H, but the ketone's sp3
    # position is the one that has to carry it.  Citing 1H spends the indicated
    # hydrogen at C1 and leaves the C3 carbonyl nothing to sit on, so the name
    # reads back with the 1,2 double bond saturated away.
    assert name_smiles("O=C1C=Cc2cccn21") == "3H-pyrrolizin-3-one"
...
def test_oxygen_carbonyl_alkenyl_aryl_substituent_tokens_keep_local_scopes():
    # An acyloxy prefix is the acyl group's name plus ``oxy``: the ``oxy`` keeps
    # the ester oxygen, the acyl phrase the acyl atoms and nothing else.
    analysis = analyze_smiles("C(=O)(OCC(=O)O)/C=C/c1ccc(Cl)cc1")
    assembly = next(step for step in analysis.decisions if step.decision == "assembled component name")
    tokens = {(token["text"], token["start"]): token for token in assembly.data["name_token_spans"]}
    name = analysis.name

    assert name == "2-(((2E)-3-(4-chlorophenyl)prop-2-enoyl)oxy)acetic acid"
    acyl_atoms = [0, 1, 7, 8, 9, 10, 11, 12, 13, 14, 15]
    assert tokens[("chlorophenyl", name.index("chlorophenyl"))]["atoms"] == acyl_atoms
    assert tokens[("prop", name.index("prop"))]["atoms"] == acyl_atoms
    assert tokens[("enoyl", name.index("enoyl"))]["atoms"] == acyl_atoms
    assert tokens[("oxy", name.index("oxy)acetic"))]["atoms"] == [2]
    assert tokens[("acetic", name.index("acetic"))]["atoms"] == [3, 4, 5, 6]
    assert tokens[("acid", name.index("acid"))]["atoms"] == [4, 5, 6]


def test_acetyloxy_substituent_tokens_bind_the_acyl_and_the_ester_oxygen():
    analysis = analyze_smiles("CC(=O)Oc1ccccc1C(=O)O")
    assembly = next(step for step in analysis.decisions if step.decision == "assembled component name")
    tokens = {(token["text"], token["start"]): token for token in assembly.data["name_token_spans"]}
    name = analysis.name

    assert name == "2-(acetyloxy)benzoic acid"
    assert tokens[("acetyl", name.index("acetyl"))]["atoms"] == [0, 1, 2]
    assert tokens[("oxy", name.index("oxy"))]["atoms"] == [3]


def test_contextual_postprocessing_updates_absorbed_binding_terms():
    bindings = [
        NameAtomBinding(stage="prefix", role="substituent", term="methyl", atom_ids={2}),
        NameAtomBinding(stage="prefix", role="substituent", term="oxy", atom_ids={0, 1}),
    ]

    result = NameAssemblyResult.from_raw_name("methyloxy", bindings, postprocess=post_process_name)

    assert result.text == "methoxy"
    assert {binding.term for binding in result.bindings} == {"methoxy", "oxy"}

    ether = NameAssemblyResult.from_raw_name(
        "methyloxy",
        [NameAtomBinding(stage="prefix", role="substituent", term="methyloxy", atom_ids={0})],
        postprocess=post_process_name,
    )

    assert ether.text == "methoxy"
    assert ether.bindings[0].term == "methoxy"
...
def test_substituted_imino_on_a_ring_centre_keeps_its_double_bond():
    ...
    assert name_smiles("CCC1CCCN(C)P1(=NC(C)(C)C)N(CC)CC") == (
        "2-(tert-butylimino)-N,N,3-triethyl-1-methyl-1-aza-2lambda^5-phosphacyclohexan-2-amine"
    )
    assert name_smiles("Cc1ccc(S(=O)(=O)N=S2CCCC3(C2)OCCO3)cc1") == (
        "7-(tosylimino)-1,4-dioxa-7lambda^4-thiaspiro[4.5]decane"
    )
...
def test_substituted_imino_does_not_steal_a_senior_nitrogen():
    ...
    assert name_smiles("NN=S1(=O)CCN(C(=O)C2CCC2)CC1") == (
        "(cyclobutyl)(1-(hydrazono)-1-oxothiomorpholin-4-yl)methanone"
    )
...
def test_sulfonimidoyl_keeps_the_ligand_beside_its_imino_nitrogen():
    ...
    assert name_smiles("CNS(=O)(=O)c1ccccc1") == "N-methylbenzenesulfonamide"
...
def test_amidinohydrazone_tail_is_part_of_principal_group():
    ...
    assert generated == "2-chloro-6-methylbenzaldehyde amidinohydrazone"


def test_hydrazone_suffix_preserves_unlocanted_ez_stereo():
    generated = name_smiles(r"N/N=C/c1ccc(N)cc1C")

    assert generated == "(E)-4-amino-2-methylbenzaldehyde hydrazone"
...
def test_display_locants_keep_numeric_order_and_rendered_text():
    ...
    numbering = {10: as_display_locant(4, "4a"), 11: as_display_locant(5)}
...
def test_charge_rules_are_data_backed():
    assert {rule.category for rule in RULES.postprocess.literal_replacements} <= {
        "grammar",
        "retained_alias",
        "morphology",
        "opsin_compat",
    ...
    assert all(rule.reason for rule in RULES.postprocess.literal_replacements)
...
def test_parser_grammar_snapshot_is_owned_runtime_data():
    ...
    assert snapshot["source"]["resource_json_dir"] == "tests/data/parser_xml_resources"
...
def test_parser_xml_resources_are_owned_json_datafiles():
    resource_dir = TEST_DATA_DIR / "parser_xml_resources"
    resource_files = sorted(resource_dir.glob("*.json"))

    assert len(resource_files) >= 39
    assert not (resource_dir / "index.json").exists()
    inline_charge = load_test_json("parser_xml_resources/inlineChargeSuffixes.json")
...
def test_stage6_to_10_emission_examples_are_generic_opsin_grammar_targets():
    examples = load_test_json("fused_emission_examples.json")
    stages = examples["stages"]

    assert examples["name_policy"] == "generic_opsin_grammar"
    assert "furo[3,2-b]thieno[2,3-e]pyridine" in stages["stage_6_systematic_fused_descriptor"]
    assert "1H-cyclopenta[l]phenanthrene" in stages["stage_7_fused_layout_numbering"]
    assert "10,5-[2,3]furanobenzo[g]quinoline" in stages["stage_8_bridged_fused"]
    assert "1'H-spiro[imidazolidine-4,2'-quinoxaline]" in stages["stage_9_spiro_wrapper"]
    assert set(stages["stage_10_charged_fused_heteroaromatics"]) == {
        "quinolin-1-ium",
        "isoquinolin-2-ium",
        "quinoline 1-oxide",
        "indol-1-ide",
...
def test_retained_fused_ring_keeps_its_name_as_a_substituent_prefix():
    ...
    cases = {
        "OCc1ccc2cccnc2c1": "(quinolin-7-yl)methanol",
        "OCc1ccnc2ccccc12": "(quinolin-4-yl)methanol",
        "OCCc1ccc2cccnc2c1": "2-(quinolin-7-yl)ethan-1-ol",
        "OCc1ccc2nccnc2c1": "(quinoxalin-6-yl)methanol",
        "OCc1ccc2nnccc2c1": "(cinnolin-6-yl)methanol",
        "OCc1ccc2ncccc2n1": "(1,5-naphthyridin-2-yl)methanol",
        "OCc1ccc2[nH]c3ccccc3c2c1": "(9H-carbazol-3-yl)methanol",
        "OCc1ccc2Oc3ccccc3Cc2c1": "(9H-xanthen-2-yl)methanol",
        "OCc1ccc2cc3ccccc3nc2c1": "(acridin-3-yl)methanol",
...
def test_pyopsin_regression_names_keep_formate_esters_principal():
    ...
    cases = {
        ...
        "NC(=[NH2+])[C-](OC=O)C#N": "1-amino-2-cyano-2-(formyloxy)ethan-2-ide-1-iminium",
    }
...
def test_diacyl_nitrogen_keeps_the_amino_parent_and_its_enclosing_marks():
    # ``-amido`` spells one acyl replacing one N-H, so a nitrogen carrying two
    # of them keeps the ``-amino`` parent.  Contracting anyway also ate the
    # multiplier's enclosing marks, leaving the unreadable ``bis2-phenyl...``.
    assert name_smiles("CC(=O)N(CCC(=O)O)C(C)=O") == "3-(diacetylamino)propanoic acid"
    assert name_smiles("CC(C(N)=O)N(C(=O)Cc1ccccc1)C(=O)Cc1ccccc1") == "2-(bis(2-phenylacetyl)amino)propanamide"

    # A single acyl still contracts, with or without an N-substituent.
    assert name_smiles("CC(=O)NCCC(=O)O") == "3-acetamidopropanoic acid"
    assert name_smiles("CC(=O)N(C)CCC(=O)O") == "3-(N-methylacetamido)propanoic acid"


def test_n_locanted_sulfonamido_branch_keeps_the_systematic_acyl_spelling():
    # The contraction rewrites the ending that implied the nitrogen into a word
    # naming a substituent on it, so an italic-N locant in front would read as a
    # substituent on *that* group's parent hydride: the contracted
    # ``N-(4-chlorophenyl)methanesulfonylcarbamoyl`` reads back with the
    # chlorophenyl moved onto the methane and the N-H restored.
    generated = name_smiles("CCOc1ccc(NC(=O)NC(=O)N(c2ccc(Cl)cc2)S(C)(=O)=O)cc1")

    assert "sulfonylcarbamoyl" not in generated
    assert generated == "N-((N-(4-chlorophenyl)methanesulfonamido)carbonyl)-N'-(4-ethoxyphenyl)urea"

    # Without the N-locant the contraction is unambiguous and still applies.
    assert name_smiles("OC(=O)c1ccc(C(=O)N)cc1") == "4-carbamoylbenzoic acid"
...
def test_connection_boundary_regression_names_keep_unambiguous_attachment():
    cases = {
        "CCN([C@H](C)CO)S(=O)(=O)c1ccccc1": "N-ethyl-N-((2R)-1-hydroxypropan-2-yl)benzenesulfonamide",
        ...
        "CC1CCN(C(=O)CN(Cc2ccc(Cl)cc2)S(=O)(=O)c2ccccc2)CC1": "N-((4-chlorophenyl)methyl)-N-(2-(4-methylpiperidin-1-yl)-2-oxoethyl)benzenesulfonamide",
...
def test_one_atom_parents_drop_the_locant_their_single_position_makes_redundant():
    ...
    assert name_smiles("OCc1ccccc1") == "phenylmethanol"
    assert name_smiles("O=C(c1ccccc1)c1ccccc1") == "diphenylmethanone"
    # ...unless the suffix brings a nitrogen of its own, which the locant is what separates it from.
    assert name_smiles("CCNC(=O)Nc1ccccc1") == "N-ethyl-N'-phenylurea"
...
def test_cyclic_sulfur_imide_substituent_keeps_ring_ligand_together():
    assert name_smiles("CC(C)(C)OC(=O)N=S1(=O)CCC1") == "tert-butyl (1-oxo-1lambda^6-thietan-1-ylidene)carbamate"
...
def test_substituted_alkoxy_prefixes_preserve_imino_ether_connectivity():
    assert name_smiles("OCCOC(=N)NC=O") == "N-(1-(2-hydroxyethoxy)-1-iminomethyl)formamide"
...
def test_acyl_with_a_competing_acid_does_not_take_the_amido_rewrite():
    ...
    assert glu_leu == "2-((2-amino-5-hydroxy-1,5-dioxopentyl)amino)-4-methylpentanoic acid"
...
def test_ketone_amidinohydrazone_keeps_its_amidino_atoms():
    ...
    assert name_smiles(r"Cc1cccc(Cl)c1/C=N\N=C(N)N") == "2-chloro-6-methylbenzaldehyde amidinohydrazone"
...
def test_enclosed_prefix_keeps_its_parentheses_beside_another_substituent():
    ...
    assert name_smiles("OCc1ccccc1OCc1ccccc1") == "(2-(benzyloxy)phenyl)methanol"
...
def test_charge_separated_sulfonyl_on_a_nitrogen_keeps_its_ligand():
    ...
    assert charge_separated == "2-benzenesulfonamido-N-methylpropanamide"
